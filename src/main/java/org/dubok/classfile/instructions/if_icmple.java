...
public class if_icmple  extends Instruction {
    ...
    public if_icmple(int offset, int opcode, final Constant[] constantPool, List<Instruction> instructions, DataInput dataInput) throws IOException {
        super(offset, opcode, constantPool, instructions);
        assert opcode == 164;
        length = 3;
        branchOffset = dataInput.readShort();
        comment = "Operand Stack: ..., value1, value2 -> ...";
        extraComment = "Branch if le, branchOffset = " + branchOffset + ", branch to instruction at offset " + (offset + branchOffset);
