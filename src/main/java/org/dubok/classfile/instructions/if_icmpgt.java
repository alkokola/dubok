...
public class if_icmpgt  extends Instruction {
    ...
    public if_icmpgt(int offset, int opcode, final Constant[] constantPool, List<Instruction> instructions, DataInput dataInput) throws IOException {
        super(offset, opcode, constantPool, instructions);
        assert opcode == 163;
        length = 3;
        branchOffset = dataInput.readShort();
        comment = "Operand Stack: ..., value1, value2 -> ...";
        extraComment = "Branch if gt, branchOffset = " + branchOffset + ", branch to instruction at offset " + (offset + branchOffset);
