...
public class if_icmpeq extends Instruction {
    ...
    public if_icmpeq(int offset, int opcode, final Constant[] constantPool, List<Instruction> instructions, DataInput dataInput) throws IOException {
        super(offset, opcode, constantPool, instructions);
        assert opcode == 159;
        length = 3;
        branchOffset = dataInput.readShort();
        comment = "Operand Stack: ..., value1, value2 -> ...";
        extraComment = "Branch if equals, branchOffset = " + branchOffset + ", branch to instruction at offset " + (offset + branchOffset);
