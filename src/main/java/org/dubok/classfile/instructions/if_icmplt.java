...
public class if_icmplt  extends Instruction {
    ...
    public if_icmplt(int offset, int opcode, final Constant[] constantPool, List<Instruction> instructions, DataInput dataInput) throws IOException {
        super(offset, opcode, constantPool, instructions);
        assert opcode == 161;
        length = 3;
        branchOffset = dataInput.readShort();
        comment = "Operand Stack: ..., value1, value2 -> ...";
        extraComment = "Branch if lt, branchOffset = " + branchOffset + ", branch to instruction at offset " + (offset + branchOffset);
